...
Kanttekening: de within-locatie SD is een ongewogen gemiddelde over locaties met wisselend aantal herhalingen (2 tot 4 jaar); bij slechts 2 metingen is een SD-schatting instabiel. Voor een robuustere, formele ontleding van de variantie (binnen-locatie, tussen-locatie-binnen-gebied, tussen-gebied) zou een variantiecomponenten-model (bv. `lmer(waarde ~ 1 + (1|Gebiedsnaam/SlootID))`) geschikter zijn.


**Koebelasting drinkende koeien**: deze afgeleide variabele geeft een proxy voor de belasting van de oever door koeien die uit de sloot drinken (relevant voor vertrapping en begrazing van de oever). De afleiding volgt een beslisregel op basis van vier brondata: aanwezigheid van afrastering langs de sloot, of koeien al dan niet uit de sloot drinken, aanwezigheid van drinkbakken, en het aantal koeien/koedagen per perceel:

- Is de sloot (deels) afgerasterd (`uitraster_perc_slootid` > 0%), dan wordt de koebelasting op 0 gezet — koeien hebben dan geen toegang tot het water.
- Is bekend dat koeien niet uit de sloot drinken (`Koeien_drinken_sloot`), dan wordt de koebelasting eveneens op 0 gezet.
...
waarbij de natte omtrek van de oever (`omtrek_nat`) een maat is voor hoeveel slootoever daadwerkelijk bereikbaar is voor drinkende koeien. Vrij-tekstwaarden voor drinkgedrag en drinkbakaanwezigheid (bijv. "ja"/"nee"/"onbekend"-varianten) zijn eerst genormaliseerd naar TRUE/FALSE/NA.
